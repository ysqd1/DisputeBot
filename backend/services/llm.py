...
def build_system_prompt(
    ...
    # 激烈程度对应的语气风格
    if aggression == 1:
        tone = "极度温和、摆事实讲道理、以理服人、语气平和"
        example = ('对方说："XX烂死了，还不如YY"\n'
                   '回答："YY确实在某些方面不错，但XX也有自己的优势，比如在XX领域YY确实不如XX，这个要看具体场景，不能一概而论"')
    elif aggression == 2:
        tone = "温和但有态度、带点情绪、但不失理智和逻辑"
        example = ('对方说："XX烂死了，还不如YY"\n'
                   '回答："你就这么确定YY比XX好？我看未必，XX起码在XX方面是真的强，你这话说的有点绝对了吧"')
    elif aggression == 3:
        tone = "中等激烈、阴阳怪气、讽刺、拐弯抹角地怼"
        example = ('对方说："XX烂死了，还不如YY"\n'
                   '回答："哦？YY就那么香？XX起码在XX方面是真的强，你怕不是云球迷吧，看比赛了吗就在这指点江山"')
    elif aggression == 4:
        tone = "很强、讽刺挖苦、阴阳怪气拉满、让人听着就想打人的程度"
        example = ('对方说："XX烂死了，还不如YY"\n'
                   '回答："我看你才是真的烂，XX好歹是XX，你YY是什么东西？跪久了站不起来是吧张口就来，云球迷滚远点别在这丢人现眼"')
    else:
        tone = "最强、直接开喷、脏话喷人、不再客气、怎么难听怎么来、语言极具攻击性和侮辱性"
        example = ('对方说："XX烂死了，还不如YY"\n'
                   '回答："你他妈的脑子被门夹了吧？XX是你能碰瓷的？你YY给XX提鞋都不配！不会看球就别在这放屁，滚回去多看几场比赛再来说话，省得在这丢人现眼让人笑掉大牙"')

    # 字数要求
    length_hint = f"约为{target_length}字" if target_length > 0 else "自由长度"

    prompt = f'''你是一个辩论助手，擅长反驳喷子。
...
- 禁止使用自问自答的反问句（如"XX？你这借口也太烂了吧！"），这种句式AI味太重
